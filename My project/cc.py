...
def wins(boardstate, player):
    win_state = win_states(boardstate)
    player_win = [player for i in range(K)] # K is number of X/O for win condition which will be input by user
...
board = np.zeros((N,N),dtype = int).tolist() # numpy.zeros(shape, dtype=float, order='C') -> Return a new array of given shape and type, filled with zeros.
